...
(()=>{
  ...
  function modalFor(modal){
    ...
    const title=modal.querySelector('.a7SmartHead h2 span:last-child');
    if(title)title.textContent='İşimi Çöz AI';
    const hint=modal.querySelector('.a7SmartHint');
    if(hint)hint.textContent='Ne yapmak istediğini kendi cümlelerinle anlat. AI önce ihtiyacını anlayıp seni doğru İşimi Çöz akışına yönlendirsin.';
    const row=modal.querySelector('.a7SmartRow');
    if(row){
      row.innerHTML='<button type="button" class="a7SmartChip" data-a7-example="Mutfaktaki musluk damlatıyor, bu hafta içinde bir usta gelip tamir etsin.">🔧 Usta / hizmet</button><button type="button" class="a7SmartChip" data-a7-example="İzmir’de bir ev almak istiyorum, tapu ve hazırlık adımlarını öğrenmek istiyorum.">🏠 Ev / tapu</button><button type="button" class="a7SmartChip" data-a7-example="İkinci el bir araba almak istiyorum, ilanlara bakmak istiyorum.">🚗 Araç</button><button type="button" class="a7SmartChip" data-a7-example="Bir kuruma başvuru için dilekçe hazırlamak istiyorum.">📄 Evrak / başvuru</button>';
      row.style.gridTemplateColumns='1fr 1fr';
    }
    const build=modal.querySelector('[data-a7-build]');
    if(build)build.textContent='Devam Et';
  }
  function routeTo(intent){
    if(intent==='service'){
      // Preserve the already-tested existing Smart Intake → Usta Bul flow.
      return {handled:false};
    }
    const modal=document.querySelector('[data-a7-smart-modal]');
    if(modal)modal.remove();
    if(intent==='property'){
      if(window.Hallet&&typeof window.Hallet.showTasinmazMenu==='function'){window.Hallet.showTasinmazMenu();return {handled:true};}
      if(typeof window.openHallet==='function')window.openHallet();
      return {handled:true};
    }
    if(intent==='document'){
      if(window.Hallet&&typeof window.Hallet.showEvrak==='function'){window.Hallet.showEvrak();return {handled:true};}
      if(typeof window.openHallet==='function')window.openHallet();
      return {handled:true};
    }
    if(intent==='official'){
      if(window.Hallet&&typeof window.Hallet.showKurum==='function'){window.Hallet.showKurum();return {handled:true};}
      if(typeof window.openHallet==='function')window.openHallet();
      return {handled:true};
    }
    if(intent==='vehicle'){
      const t=norm(localStorage.getItem('a7_ai_last_text')||'');
      if(has(t,['ilan','ilanlara bak','araç ilan','arac ilan','otomobil ilan','otomobil ilanlar'])){
        if(typeof window.showOtomobilHub==='function'){window.showOtomobilHub();return {handled:true};}
      }
      if(window.Hallet&&typeof window.Hallet.showIslemMenu==='function'){window.Hallet.showIslemMenu();return {handled:true};}
      if(typeof window.showOtomobilHub==='function'){window.showOtomobilHub();return {handled:true};}
      return {handled:true};
    }
    return {handled:false};
  }
  function showRouteResult(modal,route){
    const result=modal.querySelector('#a7SmartResult');
    if(!result)return;
    result.classList.remove('a7SmartHidden');
    result.innerHTML='<b>AI ihtiyacını anladı</b><div class="a7SmartLine"><span class="a7SmartLabel">Akış</span><strong>'+E(route.label)+'</strong></div><div style="margin-top:10px;font-size:12px;line-height:1.5">'+E(route.detail)+'</div><div class="small muted" style="margin-top:9px">Metnin kaybolmaz. Doğru bölüme geçtiğinde kaldığın yerden devam edebilirsin.</div>';
    const build=modal.querySelector('[data-a7-build]');
    if(build)build.textContent='Doğru Akışa Git';
  }
  ...
  document.addEventListener('click',function(e){
    ...
    localStorage.setItem('a7_ai_last_intent',route.intent);
    if(route.intent==='service')return; // let existing A7 Smart Intake V1/V2 handlers run unchanged
    e.preventDefault();
    e.stopImmediatePropagation();
    showRouteResult(modal,route);
    if(btn.textContent.trim()==='Doğru Akışa Git')routeTo(route.intent);
  },true);
  const observer=new MutationObserver(function(){const m=document.querySelector('[data-a7-smart-modal]');if(m)modalFor(m)});
  observer.observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(()=>{const m=document.querySelector('[data-a7-smart-modal]');if(m)modalFor(m)},200);
  window.__A7_CLASSIFY_INTENT_V1__=classify;
})();
