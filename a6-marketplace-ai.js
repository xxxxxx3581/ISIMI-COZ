/* A6.4.1 — Marketplace smart natural-language search
 * Test branch only. Client-side, read-only. Existing A3.4.1 search/filter remains untouched.
 */
(()=>{
 ...
 const state={query:null,parsed:null,active:false};
 ...
 function listingNodes(){const a=app();if(!a)return[];const buttons=[...a.querySelectorAll('button')].filter(b=>/detayı gör|detay/i.test(norm(b.textContent||'')));return buttons.map(b=>{let el=b;for(let i=0;i<10&&el&&el.parentElement;i++){el=el.parentElement;const tx=norm(el.textContent||'');if(tx.includes('try')&&tx.length<1200)return el}return b.parentElement}).filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i)}
 ...
 function rankListings(r){
  const nodes=listingNodes();if(!nodes.length)return 0;
  const scored=nodes.map(node=>({node,...cardScore(node,r)}));
  const valid=scored.filter(x=>x.score>=0);
  const hasStructured=!!(r.criteria.price||r.criteria.rooms||r.criteria.area||r.criteria.km||r.criteria.year||r.tokens.length);
  let visible=0;
  scored.sort((a,b)=>b.score-a.score);
  scored.forEach((x,i)=>{
    x.node.style.order=String(i);
    if(hasStructured){
      const good=x.score>=35;
      x.node.style.setProperty('display',good?'':'none','important');
      if(good)visible++;
    }else{x.node.style.removeProperty('display');visible++}
    let badge=x.node.querySelector('[data-a6-match]');
    if(badge)badge.remove();
    if(x.score>=35&&x.node.style.display!=='none'){
      badge=document.createElement('div');badge.dataset.a6Match='1';badge.style.cssText='font-size:11px;margin-top:5px;font-weight:800;opacity:.72';badge.textContent='✨ Eşleşiyor';x.node.appendChild(badge);
    }
  });
  if(hasStructured&&!visible&&valid.length){
    scored.sort((a,b)=>b.score-a.score).slice(0,2).forEach(x=>x.node.style.setProperty('display','','important'));
  }
  return visible;
 }
 function clearSmartVisibility(){listingNodes().forEach(n=>{n.style.removeProperty('display');const b=n.querySelector('[data-a6-match]');if(b)b.remove()});state.active=false;state.query=null;state.parsed=null}
 function enforce(){if(state.active&&state.parsed&&isBrowse())rankListings(state.parsed)}
 function runSearch(q){const s=findSearch();if(!s)return null;clearSmartVisibility();const r=parse(q);state.active=true;state.query=q;state.parsed=r;const lexical=r.tokens.filter(t=>!/^\d/.test(t)&&!/\s*m2$/.test(t)&&!/\s*km$/.test(t)&&!/\s*(hp|cc)$/.test(t)&&t!==norm(r.criteria.rooms)&&t!=='fiyat');s.value=lexical.join(' ');s.dispatchEvent(new Event('input',{bubbles:true}));s.dispatchEvent(new Event('change',{bubbles:true}));const btn=[...app().querySelectorAll('button')].find(b=>/^ara$/i.test(String(b.textContent||'').trim()));if(btn)btn.click();else s.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',bubbles:true}));[150,400,800,1400,2200].forEach(ms=>setTimeout(enforce,ms));return r}
 ...
 const ob=new MutationObserver(()=>{inject();if(state.active)setTimeout(enforce,50)});ob.observe(document.body,{childList:true,subtree:true});setInterval(enforce,500);setTimeout(inject,100);setTimeout(inject,500);setTimeout(inject,1200);
})();