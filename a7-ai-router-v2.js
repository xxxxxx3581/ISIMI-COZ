/* A7 — General AI Router V2 companion
   Adds multi-need decomposition to the existing V1 router without replacing its routing or submit flows. */
(()=>{
  ...
  function paint(modal){
    if(!modal||modal.dataset.a7V2Ready==='1')return;
  ...
  function scan(){
    ...
    const ta=modal.querySelector('#a7SmartText');
    if(!result||!ta)return;
    if(result.classList.contains('a7SmartHidden'))return;
    modal.dataset.a7V2Ready='0';
    paint(modal);
  }
  ...
  document.addEventListener('input',e=>{
    if(e.target&&e.target.id==='a7SmartText'){
      const modal=e.target.closest('[data-a7-smart-modal]');
      if(modal){modal.dataset.a7V2Ready='0';}
    }
  },true);
  setTimeout(scan,250);
  setTimeout(scan,700);
